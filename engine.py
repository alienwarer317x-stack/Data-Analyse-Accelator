...
def calculate_demand_supply_ratio(vacancy, stock, dom):
    ...
    if None in (vacancy_component, stock_component, dom_component):
        return None
    score = 0.40 * vacancy_component + 0.35 * stock_component + 0.25 * dom_component
...
BUY_GATE_EXPLANATIONS = {
    ...
    "36m Growth > 50%": "Triangulated 36‑month price growth exceeds 50%, indicating an overheated market with elevated pullback risk.",
    "10yr CAGR Alignment Issue": "Cross-source 10‑year growth estimates diverge materially, indicating potential data inconsistency. Suburb requires review.",
}
...
def build_path_to_buy(factors, failed_gates):
    ...
    for gate in failed_gates:
        ...
        if rule["type"] == "range":
            actions.append(
                f"{rule['label']} must move into the {rule['min']}–{rule['max']} range (currently {current})."
            )
        elif rule["type"] == "min":
            actions.append(
...
def evaluate_suburb(row):
    ...
    vacancy = normalise_plain(row.get("Vacancy rate") or row.get("Vacancy rate%"))
    stock = normalise_plain(row.get("Percent stock on market") or row.get("Stock on market") or row.get("Stock on market%"))
    dom = normalise_plain(row.get("Days on market") or row.get("Days on Market"))
    yield_pct = normalise_percent(row.get("Gross rental yield") or row.get("Yield %") or row.get("Gross rental yield%"))
    renters_pct = normalise_percent(row.get("Percent renters in market") or row.get("Renters %"))
    ...
    tri_36m = triangulate_36m_growth(
        sqm_36m=normalise_percent(row.get("sqm_36m_growth_pct") or row.get("sqm_36m")),
        htag_36m=normalise_percent(row.get("htag_36m_growth_pct") or row.get("htag_36m")),
        typical_36m=normalise_percent(row.get("typical_36m_growth_pct") or row.get("typical_36m")),
    )
    ...
    tri_10y = triangulate_10y_growth(
        sqm_cagr=row.get("sqm_10y_gr_pa") or row.get("sqm_10y_growth_pct"),
        oth_total_growth=row.get("oth_10y_growth") or row.get("oth_10y_growth_pct"),
        htag_total_growth=row.get("htag_10y_growth") or row.get("htag_10y_growth_pct"),
    )
    ...
    structural_data = {
        ...
        "travel_time_mins": normalise_plain(row.get("Travel Time Mins") or row.get("Average Commute Time (mins)")),
    ...
    growth_out = {**growth, **tri_10y, "avg_36m": tri_36m.get("avg_36m"), "status_36m": tri_36m.get("status")}
    narrative = build_authoritative_narrative(
        decision=decision,
        dsr=demand_supply,
        growth=growth_out,
        failed_gates=failed,
        structural_eval=structural_stage3,
        factors=factors,
    )

    failed_out = failed if failed else ["None"]
    return {
        "Decision": decision,
